    ...
    def ban_user(self, user_id: int, admin_id: int, reason: str = "Violation of terms"):
        ...
        with self._lock:
            try:
                ...
                cursor.execute('''
                    INSERT OR REPLACE INTO user_preferences 
                    (user_id, is_banned, ban_reason, ban_date, updated_at)
                    VALUES (?, ?, ?, ?, ?)
                ''', (user_id, 1, reason, datetime.utcnow(), datetime.utcnow()))
                ...
                self.conn.commit()
                logger.info(f"Banned user {user_id} by admin {admin_id}")
                return True
        ...
        with self._lock:
            try:
                ...
                cursor.execute('''
                    ...
                    SET is_banned = 0, ban_reason = NULL, ban_date = NULL, updated_at = ?
                    WHERE user_id = ?
                ...
                self.conn.commit()
                logger.info(f"Unbanned user {user_id} by admin {admin_id}")
                return True
